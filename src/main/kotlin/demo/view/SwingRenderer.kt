...
class SwingRenderer<T : Piece>(private val scope: CoroutineScope) : JPanel(), GameRenderer<T> {
    ...
    private fun subscribeToGameEvents() {
        ...
        EventOrchestrator.subscribe<PlayerAPMUpdated> { event ->
            if (event.gameId == gameId) {
                playerAPM = event.apm
            }
        }
    ...
    private fun drawGameBoard(graphics: Graphics2D, snapshot: GameSnapshot<T>) {
        ...
        drawPendingGarbageIndicator(graphics, snapshot, boardOffsetX, uiOffsetY)
    ...
    private fun drawPendingGarbageIndicator(
        ...
        val boardPixelHeight = snapshot.board.visibleRows * blockSize
        val lineHeight = (pendingLines.coerceAtMost(snapshot.board.visibleRows)) * blockSize

        val leftRailX = offsetX - BOARD_BORDER_WIDTH - 6
        val bottomY = offsetY + boardPixelHeight

        graphics.color = Color.RED
        graphics.fillRect(leftRailX - 2, bottomY - lineHeight, 4, lineHeight)
    }
    ...
    private fun drawHUD(graphics: Graphics2D, hudX: Int, hudY: Int): Int {
        ...
        if (playerAPM > 0f) {
            ...
            currentY += HUD_LINE_HEIGHT
            drawHUDLine(graphics, playerAPM.toInt().toString(), hudX + HUD_PADDING, currentY)
        }
        return currentY
    }
