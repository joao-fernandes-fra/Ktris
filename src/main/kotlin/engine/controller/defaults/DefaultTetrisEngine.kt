...
abstract class DefaultTetrisEngine<T : Piece>(
    ...
    private fun setupInputEvents() {
        ...
        subscribeForGame<SlowDownTime, Float>(
            { duration -> onTimeState(TimeState.SLOWED, duration) },
            { it.duration }
        )
        subscribeForGame<FreezeTime, Float>(
    ...
    open suspend fun update(deltaTime: Float) {
    ...
    override suspend fun onTimeState(timeState: TimeState, duration: Float) {
        scope.launch {
